...
def _resolve_user_id(user_id: Optional[str]) -> str:
    candidate = (user_id or DEFAULT_USER_ID or "").strip()
    if not candidate:
        raise HTTPException(status_code=400, detail="user_id is required; set DEFAULT_USER_ID or pass ?user_id=")
    return candidate
...
def goal_setting_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        return {"goal_setting_completed": goal_setting_completed(resolved_user_id)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/customer-app-registration-completed")
def registration_completed_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        result = customer_app_registration_completed(events)
...
def login_completed_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        return {"customer_app_login_completed": customer_app_login_completed(events)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/customer-app-engaged")
def engaged_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        return {"customer_app_engaged": customer_app_engaged(events)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/customer-app-engagement-dropoff")
def engagement_dropoff_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        return {"customer_app_engagement_dropoff": customer_app_engagement_dropoff(events)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/customer-app-retained")
def retained_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        return {"customer_app_retained": customer_app_retained(events)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/customer-app-retained-dropoff")
def retained_dropoff_endpoint(user_id: Optional[str] = Query(default=None)) -> Dict[str, bool]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        events = fetch_events(resolved_user_id)
        return {"customer_app_retained_dropoff": customer_app_retained_dropoff(events)}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.get("/signals")
def signals_summary(user_id: Optional[str] = Query(default=None)) -> Dict[str, Any]:
    resolved_user_id = _resolve_user_id(user_id)
    try:
        summary = build_signal_summary(resolved_user_id)
